...
type CreateQuoteStatus = "draft" | "pending";
...
export default function AgentQuoteReviewPage() {
  ...
  const [submitMode, setSubmitMode] = useState<CreateQuoteStatus | null>(null);

  const customTripDays = useTripBuilderStore((state) => state.customTripDays) as
    | StoreDay[]
    | undefined;
  ...
  const storedCustomerName = useTripBuilderStore((state: any) => state.customerName ?? "");
  const storedCustomerEmail = useTripBuilderStore((state: any) => state.customerEmail ?? "");
  const storedCustomerPhone = useTripBuilderStore((state: any) => state.customerPhone ?? "");
  const storedNotes = useTripBuilderStore((state: any) => state.quoteNotes ?? state.notes ?? "");

  const [customerName, setCustomerName] = useState(storedCustomerName);
  const [customerEmail, setCustomerEmail] = useState(storedCustomerEmail);
  ...
  const travellersLabel = `${safeNumber(adults)} Adult${safeNumber(adults) === 1 ? "" : "s"}${
    safeNumber(children) > 0
      ? `, ${safeNumber(children)} Child${safeNumber(children) === 1 ? "" : "ren"}`
      : ""
  }`;

  async function handleCreateQuote(status: CreateQuoteStatus) {
    setSubmitError("");
    ...
    if (status === "pending") {
      if (!customerName.trim()) {
        setSubmitError("Customer name is required before creating the quote.");
        return;
      }

      if (!customerPhone.trim()) {
        setSubmitError("Customer phone is required before creating the quote.");
        return;
      }
    }
    ...
    try {
      ...
      const createdQuoteId = result?.quote?.id;

      if (createdQuoteId) {
        router.push(`/agent/quotes/${createdQuoteId}`);
        return;
      }
  ...
  return (
    <PortalShell
      title="Quote Review"
      subtitle="Finalize itinerary, customer details, and pricing before saving the quote."
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-[24px] border border-slate-200 bg-white p-4 md:col-span-2">
                <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-slate-900">
                  ...
                  Customer information for this quote
                </div>
                <p className="text-sm text-slate-600">
                  Save as Draft allows partial details. Create Quote requires at least customer name and phone.
                </p>
              </div>
              ...
              <label className="space-y-2 md:col-span-2">
                <span className="text-sm font-medium text-slate-700">Quote Notes</span>
                <textarea
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder="Add internal or customer-facing note for this quote"
                  rows={4}
                  className="w-full rounded-[18px] border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-slate-400"
                />
              </label>
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-sm text-slate-700">
                    ...
                    <span className="font-semibold text-slate-950">
                      {formatCurrency(resolvedHotelTotal)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-sm text-slate-700">
                    <span>Transfers</span>
                    <span className="font-semibold text-slate-950">
                      {formatCurrency(resolvedTransferTotal)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-sm text-slate-700">
                    <span>Sightseeing</span>
                    <span className="font-semibold text-slate-950">
                      {formatCurrency(resolvedSightseeingTotal)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-sm text-slate-700">
                    <span>Meals</span>
                    <span className="font-semibold text-slate-950">
                      {formatCurrency(resolvedMealsTotal)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-sm text-slate-700">
                    <span>Extras</span>
                    <span className="font-semibold text-slate-950">
                      {formatCurrency(resolvedExtrasTotal)}
                    </span>
                  </div>
                  ...
                  {resolvedServiceFee > 0 ? (
                    <div className="flex items-center justify-between text-sm text-slate-700">
                      ...
                      <span className="font-semibold text-slate-950">
                        {formatCurrency(resolvedServiceFee)}
                      </span>
                    </div>
                  ) : null}
                  ...
                  {resolvedMarkupTotal > 0 ? (
                    <div className="flex items-center justify-between text-sm text-slate-700">
                      <span>Markup</span>
                      <span className="font-semibold text-slate-950">
                        {formatCurrency(resolvedMarkupTotal)}
                      </span>
                    </div>
                  ) : null}
              ...
              <div className="rounded-[26px] border border-slate-200 bg-slate-50 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-900">Quote Actions</p>
                    <p className="mt-1 text-sm leading-6 text-slate-600">
                      Save as Draft to capture progress now, or Create Quote to finalize and move ahead.
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-4 grid gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => handleCreateQuote("pending")}
                    disabled={isSubmitting}
                  ...
                  >
                    {isSubmitting && submitMode === "pending" ? (
                      <>
                        <Loader2 className="h-4 w-4 animate-spin" />
                        Creating Quote...
                      </>
